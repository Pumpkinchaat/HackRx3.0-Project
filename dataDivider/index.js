...
const main = async () => {
  ...
  const positiveReviews = await ProcessedReview.find({ rating: { $gt: 2.5 } });
  const negativeReviews = await ProcessedReview.find({ rating: { $lte: 2.5 } }); //2.5 will be considered a negative review
